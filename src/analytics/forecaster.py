...
class IncidentForecaster:
    ...
    def train(
        self, historical_data: List[Dict[str, Any]], test_size: float = 0.2
    ) -> Dict[str, float]:
    ...
    async def predict(
        ...
        for future_date in future_dates:
            ...
            features = {
                ...
                "lag_7": recent_df["incident_count"].iloc[-7]
                if len(recent_df) >= 7
                else recent_df["incident_count"].mean(),
                "lag_30": recent_df["incident_count"].iloc[-30]
                if len(recent_df) >= 30
                else recent_df["incident_count"].mean(),
            ...
            new_row = pd.DataFrame(
                [{"timestamp": future_date, "incident_count": prediction, **features}]
            )
            df = pd.concat([df, new_row], ignore_index=True)

        duration = (time.perf_counter() - start_time) * 1000

        logger.info("Forecast generated: days=%d, duration=%.2fms", days_ahead, duration)
