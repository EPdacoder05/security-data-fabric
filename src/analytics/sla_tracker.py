...
class SLATracker:
    ...
    async def check_sla_status(
        ...
        if sla_record['resolved_at']:
            status = SLAStatus.MET if sla_record['status'] == SLAStatus.MET.value else SLAStatus.BREACHED
    ...
    async def resolve_incident(
        ...
        return {
            ...
            'variance_minutes': round(
                sla_record['actual_response_minutes'] - sla_record['target_response_minutes'],
                2
            ),
        ...
        for incident_id in list(self.active_slas.keys()):
            ...
            if sla_status['is_breached']:
                breaches.append({
                    ...
                    'breach_time_minutes': sla_status['elapsed_minutes'] - sla_status['target_minutes']
                })
    ...
    async def get_sla_metrics(
        ...
        resolved_incidents = [
            sla for sla in self.active_slas.values()
        ...
        sla_met_count = sum(
            1 for sla in resolved_incidents
            if sla['status'] == SLAStatus.MET.value
        )
        ...
        avg_response_time = sum(
            sla['actual_response_minutes'] for sla in resolved_incidents
        ) / total
        
        metrics_by_severity = {}
        for severity in SLASeverity:
            severity_incidents = [
                sla for sla in resolved_incidents
                if sla['severity'] == severity.value
            ]
