...
class ComplianceReporter:
    ...
    async def get_iso27001_compliance(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        metrics["training_compliant"] = (
            metrics["training_completion_rate"] >= self.ISO27001_TRAINING_COMPLETION
        )
    ...
    async def get_gdpr_compliance(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        oldest_audit_result = await db.execute(select(func.min(AuditLog.timestamp)))
    ...
    async def get_compliance_dashboard(
        self, db: AsyncSession, frameworks: Optional[List[ComplianceFramework]] = None
    ) -> Dict[str, Any]:
